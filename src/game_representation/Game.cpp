...
Window& Game::GetWindow() { return representation::Window::Instance(); }
        ...
        while (!(world->gameOver || GetWindow().IsDone())) {
                ...
                if (logic::utility::Stopwatch::Instance().getDeltaTime() < (1.0f / getFrameRate())) {
                        std::chrono::milliseconds ms = std::chrono::milliseconds(
                            (int)(((1.0f / getFrameRate()) - logic::utility::Stopwatch::Instance().getDeltaTime()) *
                                  1000));
...
void Game::createTexts()
{
        ...
        createSingleText(menuText, font1, 20, sf::Color::Red, sf::Color::Yellow, 3,
                         std::make_pair(150.0f, (float)representation::Window::Instance().GetWindowSize().y / 3),
                         "Press Enter to start the game.");
        createSingleText(menuText0, font1, 80, sf::Color::Green, sf::Color::Yellow, 1,
                         std::make_pair(70.0f, (float)representation::Window::Instance().GetWindowSize().y / 8),
                         "Doodle Jump");
        createSingleText(currentHighScore, font1, 40, sf::Color::Blue, sf::Color::Green, 2,
                         std::make_pair(60.0f, (float)representation::Window::Instance().GetWindowSize().y / 1.5f), "");
        createSingleText(allTimeHighScore, font1, 40, sf::Color::Blue, sf::Color::Red, 2,
                         std::make_pair(60.0f, (float)representation::Window::Instance().GetWindowSize().y / 1.3f), "");
}

void Game::createSingleText(sf::Text& _text, const sf::Font& _font, int charSize, sf::Color fillColor,
...
void Game::drawPlayer() { representation::Window::Instance().Draw(cFactory->playerViews[0]->getSprite()); }
