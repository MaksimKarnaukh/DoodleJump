...
EntityView::EntityView(const std::shared_ptr<logic::EntityModel>& entityModel) { mEntityModel = entityModel; }
...
void EntityView::update()
{
        ...
        if (!mEntityModel.expired()) {
                ...
                std::pair<int, int> pixels = logic::utility::Camera::Instance().projectCoordinates(
                    p->getPositionX(), p->getPositionY()); // projection
