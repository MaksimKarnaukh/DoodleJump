...
void World::receiveInput(std::string& key)
{
        ...
        if (key == "Left") {
                ...
                if ((doodle->getPositionX() + doodle->getWidth() / 2 < leftBound)) { // links buiten window
                        doodle->setPositionX(rightBound -
                                             doodle->getWidth() / 2); // we zetten onze doodle aan de rechterkant
                }
        } else if (key == "Right") {
                doodle->moveRight();
                if ((doodle->getPositionX() > rightBound - doodle->getWidth() / 2)) { // rechts buiten window
                        doodle->setPositionX(leftBound -
                                             doodle->getWidth() / 2); // we zetten onze doodle aan de linkerkant
                }
        }
}
...
void World::createEntities()
{
        ...
        float factor = std::max(
            0.25f, (1.0f - (std::floor(doodle->getPositionY() / platformGen_interval)) /
                               100)); // represents the depending factor on the score (y-coordinate of our player)
        ...
        if (formerPlatformPosY + 0.05 < platformYPos) {
                ...
                if (isCreate) {
                        ...
                        float factorBonus =
                            std::min(0.08f, (std::floor(doodle->getPositionY() / bonusGen_interval)) /
                                                100); // per 500 height, the chance for a bonus gets decreased by 0.01
                                                      // with at the end a minimum value of 0.02.
                        bool isCreateBonus = false;

                        switch (pl) {
                        case 0:
                                platform = Factory->createStaticPlatform(platformXPos, platformYPos, platformWidth,
                                                                         platformHeight);

                                isCreateBonus = logic::utility::Random::Instance().bernoulliDistribution(
                                    chanceForBonus - factorBonus);
                                if (isCreateBonus) {
                                ...
                                break;
                        case 1:
                                platform = Factory->createTemporaryPlatform(platformXPos, platformYPos, platformWidth,
                                                                            platformHeight);
                                break;
                        case 2:
                                platform = Factory->createVerticalPlatform(platformXPos, platformYPos, platformWidth,
                                                                           platformHeight);
                                break;
                        case 3:
                                platform = Factory->createHorizontalPlatform(platformXPos, platformYPos, platformWidth,
                                                                             platformHeight);
                                break;
                        default:
                                std::cout << "No platform was created"
                                          << std::endl; // default statement used for error output
                        }
                        ...
                        if (isCreateBonus) {
                                float bonusXPos = logic::utility::Random::Instance().uniformRealDistribution(
                                    platformXPos + 0.01f,
                                    platformXPos + platform->getWidth() - bonus->getWidth() - 0.01f);
...
void World::update()
{
        ...
        if (doodle->getPositionY() >= shiftBorder &&
            doodle->getPositionY() >= logic::utility::Camera::Instance().getShiftValue() + shiftBorder) {
...
bool World::checkForUndetectedCollision(const std::shared_ptr<logic::EntityModel>& pl,
                                        std::vector<std::pair<float, float>>& middleLine,
                                        const std::shared_ptr<logic::EntityModel>& pl2) const
{
        ...
        for (int bg = bgTiles.size() - 1; bg >= 0; bg--) {
                for (auto i = 0; i < bgTiles[bg].size(); i++) {
                        for (auto j = 0; j < bgTiles[bg][i]->getObservers().size(); j++) {
                                bgTiles[bg][i]->removeObserver(
                                    (bgTiles[bg][i]->getObservers()[j])); // remove all observers
                        }
                }
        ...
        for (auto c = 0; c < (1.5 / bgTileSideLength) + 30.0;
             c++) { // +35.0, otherwise at low fps, we get visual problems.
                ...
                for (auto r = 0; r < (rightBound / bgTileSideLength); r++) {
                        std::shared_ptr<logic::BGTile> bgTile = Factory->createBGTile(
                            (float)(r * bgTileSideLength), float((c + 1) * 0.03), bgTileSideLength, bgTileSideLength);
...
float World::calculatePlatformXPos(const std::shared_ptr<logic::Platform>& platform) const
{
        ...
        if (wasBonusGenerated) {
                float platformXPos1 = logic::utility::Random::Instance().uniformRealDistribution(
                    leftBound + 0.02f, formerPlatformPosX - platform->getWidth() - 0.08f);
                float platformXPos2 = logic::utility::Random::Instance().uniformRealDistribution(
                    formerPlatformPosX + platform->getWidth() + 0.08f, rightBound - platform->getWidth() - 0.02f);
                ...
                platformXPos = logic::utility::Random::Instance().uniformRealDistribution(
                    leftBound + 0.02f, rightBound - platform->getWidth() - 0.02f);
        }
        return platformXPos;
}
...
void World::checkForCollisionBetweenPlatforms()
{
        ...
        for (auto pl0 = 0; pl0 < platforms.size(); pl0++) {
                for (auto pl1 = 0; pl1 < platforms.size(); pl1++) {
                        if (pl0 != pl1) {
                                if (platforms[pl0]->getPositionX() + platforms[pl0]->getWidth() >=
                                        platforms[pl1]->getPositionX() &&
                                    platforms[pl0]->getPositionX() <=
                                        platforms[pl1]->getPositionX() + platforms[pl1]->getWidth()) {

                                        // checken op de y-coordinaten
                                        if (platforms[pl0]->getPositionY() >=
                                                platforms[pl1]->getPositionY() - platforms[pl1]->getHeight() &&
                                            platforms[pl0]->getPositionY() - platforms[pl0]->getHeight() <=
                                                platforms[pl1]->getPositionY()) {
