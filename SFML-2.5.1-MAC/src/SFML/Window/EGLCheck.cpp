...
void eglCheckError(const char* file, unsigned int line, const char* expression)
{
        ...
        if (errorCode != EGL_SUCCESS) {
                ...
                case EGL_BAD_ACCESS: {
                        ...
                        description = "EGL cannot access a requested resource (for example, a context is bound in "
                                      "another thread)";
                        break;
                }
                ...
                case EGL_BAD_CURRENT_SURFACE: {
                        ...
                        description = "the current surface of the calling thread is a window, pbuffer, or pixmap that "
                                      "is no longer valid";
                        break;
                }
                ...
                case EGL_BAD_DISPLAY: {
                        ...
                        description = "an EGLDisplay argument does not name a valid EGLDisplay; or, EGL is not "
                                      "initialized on the specified EGLDisplay";
                        break;
                }

                case EGL_BAD_SURFACE: {
                        error = "EGL_BAD_SURFACE";
                        description = "an EGLSurface argument does not name a valid surface (window, pbuffer, or "
                ...
                case EGL_BAD_MATCH: {
                        error = "EGL_BAD_MATCH";
                        description = "arguments are inconsistent; for example, an otherwise valid context requires "
                ...
                case EGL_CONTEXT_LOST: {
                        ...
                        description = "a power management event has occurred. The application must destroy all "
                ...
                // Log the error
                err() << "An internal EGL call failed in " << fileString.substr(fileString.find_last_of("\\/") + 1)
                      << " (" << line << ") : "
                      << "\nExpression:\n   " << expression << "\nError description:\n   " << error << "\n   "
                      << description << "\n"
                      << std::endl;
