...
bool ewmhSupported()
{
        ...
        result = XGetWindowProperty(display, rootWindow, netSupportingWmCheck, 0, 1, False, XA_WINDOW, &actualType,
        ...
        result = XGetWindowProperty(display, rootWindow, netWmName, 0, 0x7fffffff, False, utf8StringType, &actualType,
                                    &actualFormat, &numItems, &numBytes, &data);
...
bool getEWMHFrameExtents(::Display* disp, ::Window win, long& xFrameExtent, long& yFrameExtent)
{
        ...
        int result = XGetWindowProperty(disp, win, frameExtents, 0, 4, False, XA_CARDINAL, &actualType, &actualFormat,
                                        &numItems, &numBytesLeft, &data);
...
WindowImplX11::WindowImplX11(WindowHandle handle)
    : m_window(0), m_screen(0), m_inputMethod(NULL), m_inputContext(NULL), m_isExternal(true), m_oldVideoMode(0),
      m_oldRRCrtc(0), m_hiddenCursor(0), m_lastCursor(None), m_keyRepeat(true), m_previousSize(-1, -1),
      m_useSizeHints(false), m_fullscreen(false), m_cursorGrabbed(false), m_windowMapped(false), m_iconPixmap(0),
      m_iconMaskPixmap(0), m_lastInputTime(0)
{
...
WindowImplX11::WindowImplX11(VideoMode mode, const String& title, unsigned long style, const ContextSettings& settings)
    : m_window(0), m_screen(0), m_inputMethod(NULL), m_inputContext(NULL), m_isExternal(false), m_oldVideoMode(0),
      m_oldRRCrtc(0), m_hiddenCursor(0), m_lastCursor(None), m_keyRepeat(true), m_previousSize(-1, -1),
      m_useSizeHints(false), m_fullscreen((style & Style::Fullscreen) != 0), m_cursorGrabbed(m_fullscreen),
      m_windowMapped(false), m_iconPixmap(0), m_iconMaskPixmap(0), m_lastInputTime(0)
{
        // Open a connection with the X server
        m_display = OpenDisplay();
        ...
        m_window =
            XCreateWindow(m_display, DefaultRootWindow(m_display), windowPosition.x, windowPosition.y, width, height, 0,
                          depth, InputOutput, visual, CWEventMask | CWOverrideRedirect | CWColormap, &attributes);
        ...
        if (!m_fullscreen) {
                ...
                if (WMHintsAtom) {
                        ...
                        XChangeProperty(m_display, m_window, WMHintsAtom, WMHintsAtom, 32, PropModeReplace,
...
////////////////////////////////////////////////////////////
WindowHandle WindowImplX11::getSystemHandle() const { return m_window; }
...
Vector2i WindowImplX11::getPosition() const
{
        ...
        XTranslateCoordinates(m_display, m_window, DefaultRootWindow(m_display), 0, 0, &xAbsRelToRoot, &yAbsRelToRoot,
                              &child);
...
void WindowImplX11::setTitle(const String& title)
{
        ...
        XChangeProperty(m_display, m_window, wmName, useUtf8, 8, PropModeReplace, utf8Title.c_str(), utf8Title.size());
        ...
        XChangeProperty(m_display, m_window, wmIconName, useUtf8, 8, PropModeReplace, utf8Title.c_str(),
                        utf8Title.size());

// Set the non-Unicode title as a fallback for window managers who don't support _NET_WM_NAME.
#ifdef X_HAVE_UTF8_STRING
        Xutf8SetWMProperties(m_display, m_window, title.toAnsiString().c_str(), title.toAnsiString().c_str(), NULL, 0,
                             NULL, NULL, NULL);
#else
        XmbSetWMProperties(m_display, m_window, title.toAnsiString().c_str(), title.toAnsiString().c_str(), NULL, 0,
                           NULL, NULL, NULL);
#endif
}
...
void WindowImplX11::setIcon(unsigned int width, unsigned int height, const Uint8* pixels)
{
        ...
        XImage* iconImage =
            XCreateImage(m_display, defVisual, defDepth, ZPixmap, 0, (char*)iconPixels, width, height, 32, 0);
        if (!iconImage) {
        ...
        m_iconMaskPixmap =
            XCreatePixmapFromBitmapData(m_display, m_window, (char*)&maskPixels[0], width, height, 1, 0, 1);
        ...
        *ptr++ = height;
        ...
        XChangeProperty(m_display, m_window, netWmIcon, XA_CARDINAL, 32, PropModeReplace,
...
void WindowImplX11::setMouseCursorGrabbed(bool grabbed)
{
        ...
        if (grabbed) {
                ...
                for (unsigned int trial = 0; trial < maxTrialsCount; ++trial) {
                        int result = XGrabPointer(m_display, m_window, True, None, GrabModeAsync, GrabModeAsync,
...
////////////////////////////////////////////////////////////
void WindowImplX11::setKeyRepeatEnabled(bool enabled) { m_keyRepeat = enabled; }
...
void WindowImplX11::grabFocus()
{
        ...
        if (netActiveWindow) {
                ...
                int result = XSendEvent(m_display, DefaultRootWindow(m_display), False,
...
void WindowImplX11::setVideoMode(const VideoMode& mode)
{
        ...
        if (!res) {
                err() << "Failed to get the current screen resources for fullscreen mode, switching to window mode"
                      << std::endl;
                return;
        }
        ...
        XRRSetCrtcConfig(m_display, res, outputInfo->crtc, CurrentTime, crtcInfo->x, crtcInfo->y, xRandMode,
...
void WindowImplX11::resetVideoMode()
{
        if (fullscreenWindow == this) {
                ...
                if (checkXRandR(xRandRMajor, xRandRMinor)) {
                        ...
                        XRRSetCrtcConfig(m_display, res, m_oldRRCrtc, CurrentTime, crtcInfo->x, crtcInfo->y,
                                         m_oldVideoMode, crtcInfo->rotation, &output, 1);
...
void WindowImplX11::switchToFullscreen()
{
        ...
        if (ewmhSupported()) {
                ...
                if (netWmBypassCompositor) {
                        ...
                        XChangeProperty(m_display, m_window, netWmBypassCompositor, XA_CARDINAL, 32, PropModeReplace,
...
void WindowImplX11::setProtocols()
{
        ...
        if (netWmPing && netWmPid) {
                ...
                XChangeProperty(m_display, m_window, netWmPid, XA_CARDINAL, 32, PropModeReplace,
        ...
        if (!atoms.empty()) {
                XChangeProperty(m_display, m_window, wmProtocols, XA_ATOM, 32, PropModeReplace,
...
void WindowImplX11::initialize()
{
        ...
        if (m_inputMethod) {
                m_inputContext = XCreateIC(m_inputMethod, XNClientWindow, m_window, XNFocusWindow, m_window,
        ...
        if (wmWindowType && wmWindowTypeNormal) {
                XChangeProperty(m_display, m_window, wmWindowType, XA_ATOM, 32, PropModeReplace,
...
void WindowImplX11::updateLastInputTime(::Time time)
{
        if (time && (time != m_lastInputTime)) {
                ...
                if (netWmUserTime) {
                        XChangeProperty(m_display, m_window, netWmUserTime, XA_CARDINAL, 32, PropModeReplace,
...
bool WindowImplX11::processEvent(XEvent& windowEvent)
{
        ...
        if (windowEvent.type == KeyRelease) {
                ...
                std::deque<XEvent>::iterator iter = std::find_if(
                    m_events.begin(), m_events.end(), KeyRepeatFinder(windowEvent.xkey.keycode, windowEvent.xkey.time));
        ...
        case FocusIn: {
                ...
                // If the window has been previously marked urgent (notification) as a result of a focus request, undo
                // that
                XWMHints* hints = XGetWMHints(m_display, m_window);
                if (hints != NULL) {
        ...
        case KeyPress: {
                ...
                if (!XFilterEvent(&windowEvent, None)) {
                        ...
                        if (m_inputContext) {
                                ...
                                int length = Xutf8LookupString(m_inputContext, &windowEvent.xkey,
...
////////////////////////////////////////////////////////////
RROutput WindowImplX11::getOutputPrimary(::Window& rootWindow, XRRScreenResources* res, int xRandRMajor,
                                         int xRandRMinor)
{
