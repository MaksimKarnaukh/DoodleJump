...
String getErrorString(DWORD errorCode)
{
        ...
        if (FormatMessage(FORMAT_MESSAGE_MAX_WIDTH_MASK | FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                          ...
                          NULL, errorCode, 0, reinterpret_cast<LPTSTR>(&buffer), 256, NULL) != 0) {
...
WglContext::WglContext(WglContext* shared)
    : m_window(NULL), m_pbuffer(NULL), m_deviceContext(NULL), m_context(NULL), m_ownsWindow(false)
{
        ensureInit();
...
////////////////////////////////////////////////////////////
WglContext::WglContext(WglContext* shared, const ContextSettings& settings, const WindowImpl* owner,
...
void WglContext::setVerticalSyncEnabled(bool enabled)
{
        ...
        if (SF_GLAD_WGL_EXT_swap_control) {
                if (wglSwapIntervalEXT(enabled ? 1 : 0) == FALSE)
                        err() << "Setting vertical sync failed: " << getErrorString(GetLastError()).toAnsiString()
                              << std::endl;
        } else {
                                      ...
                                      bool pbuffer)
{
        ...
        if (SF_GLAD_WGL_ARB_pixel_format) {
                ...
                UINT nbFormats;
                bool isValid =
                    wglChoosePixelFormatARB(deviceContext, intAttributes, NULL, 512, formats, &nbFormats) != FALSE;

                if (!isValid)
                        err() << "Failed to enumerate pixel formats: " << getErrorString(GetLastError()).toAnsiString()
                              << std::endl;
                ...
                if (isValid && (nbFormats > 0)) {
                        ...
                        for (UINT i = 0; i < nbFormats; ++i) {
                                ...
                                int values[7];
                                const int attributes[] = {WGL_RED_BITS_ARB,    WGL_GREEN_BITS_ARB, WGL_BLUE_BITS_ARB,
                                                          WGL_ALPHA_BITS_ARB,  WGL_DEPTH_BITS_ARB, WGL_STENCIL_BITS_ARB,
                                                          WGL_ACCELERATION_ARB};

                                if (wglGetPixelFormatAttribivARB(deviceContext, formats[i], PFD_MAIN_PLANE, 7,
                                                                 attributes, values) == FALSE) {
                                ...
                                if (SF_GLAD_WGL_ARB_multisample) {
                                        ...
                                        if (wglGetPixelFormatAttribivARB(deviceContext, formats[i], PFD_MAIN_PLANE, 2,
                                                                         sampleAttributes, sampleValues) == FALSE) {
                                ...
                                if (SF_GLAD_WGL_ARB_framebuffer_sRGB || SF_GLAD_WGL_EXT_framebuffer_sRGB) {
                                        ...
                                        if (wglGetPixelFormatAttribivARB(deviceContext, formats[i], PFD_MAIN_PLANE, 1,
                                ...
                                if (pbuffer) {
                                        ...
                                        if (wglGetPixelFormatAttribivARB(deviceContext, formats[i], PFD_MAIN_PLANE, 1,
                                                                         pbufferAttributes, &pbufferValue) == FALSE) {
                                ...
                                int score =
                                    evaluateFormat(bitsPerPixel, settings, color, values[4], values[5],
                                                   sampleValues[0] ? sampleValues[1] : 0,
                                                   values[6] == WGL_FULL_ACCELERATION_ARB, sRgbCapableValue == TRUE);
...
void WglContext::updateSettingsFromPixelFormat()
{
        ...
        if (format == 0) {
                err() << "Failed to get selected pixel format: " << getErrorString(GetLastError()).toAnsiString()
                      << std::endl;
                return;
        }
        ...
        if (SF_GLAD_WGL_ARB_pixel_format) {
                ...
                if (SF_GLAD_WGL_ARB_multisample) {
                        ...
                        if (wglGetPixelFormatAttribivARB(m_deviceContext, format, PFD_MAIN_PLANE, 2, sampleAttributes,
                                                         sampleValues) == TRUE) {
                ...
                if (SF_GLAD_WGL_ARB_framebuffer_sRGB || SF_GLAD_WGL_EXT_framebuffer_sRGB) {
                        ...
                        if (wglGetPixelFormatAttribivARB(m_deviceContext, format, PFD_MAIN_PLANE, 1,
                                                         &sRgbCapableAttribute, &sRgbCapableValue) == TRUE) {
...
void WglContext::createSurface(WglContext* shared, unsigned int width, unsigned int height, unsigned int bitsPerPixel)
{
        ...
        if (!m_deviceContext) {
                ...
                m_window = CreateWindowA("STATIC", "", WS_POPUP | WS_DISABLED, 0, 0, width, height, NULL, NULL,
