...
std::string getErrorString(DWORD error)
{
        ...
        if (FormatMessage(FORMAT_MESSAGE_MAX_WIDTH_MASK | FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                          NULL, error, 0, reinterpret_cast<PTCHAR>(&buffer), 0, NULL) == 0)
...
sf::String getDeviceName(unsigned int index, JOYCAPS caps)
{
        ...
        result =
            RegQueryValueEx(currentKey, subkey.c_str(), NULL, NULL, reinterpret_cast<LPBYTE>(keyData), &keyDataSize);
        ...
        result = RegQueryValueEx(currentKey, REGSTR_VAL_JOYOEMNAME, NULL, NULL, reinterpret_cast<LPBYTE>(keyData),
                                 &keyDataSize);
        RegCloseKey(currentKey);

        if (result != ERROR_SUCCESS) {
                sf::err() << "Unable to query name for joystick at index " << index << ": " << getErrorString(result)
                          << std::endl;
                return joystickDescription;
        }
...
////////////////////////////////////////////////////////////
void JoystickImpl::setLazyUpdates(bool status) { lazyUpdates = status; }
...
////////////////////////////////////////////////////////////
Joystick::Identification JoystickImpl::getIdentification() const { return m_identification; }
...
JoystickState JoystickImpl::update()
{
        ...
        JOYINFOEX pos;
        pos.dwFlags =
            JOY_RETURNX | JOY_RETURNY | JOY_RETURNZ | JOY_RETURNR | JOY_RETURNU | JOY_RETURNV | JOY_RETURNBUTTONS;
        ...
        if (joyGetPosEx(m_index, &pos) == JOYERR_NOERROR) {
                ...
                // Axes
                state.axes[Joystick::X] =
                    (pos.dwXpos - (m_caps.wXmax + m_caps.wXmin) / 2.f) * 200.f / (m_caps.wXmax - m_caps.wXmin);
                state.axes[Joystick::Y] =
                    (pos.dwYpos - (m_caps.wYmax + m_caps.wYmin) / 2.f) * 200.f / (m_caps.wYmax - m_caps.wYmin);
                state.axes[Joystick::Z] =
                    (pos.dwZpos - (m_caps.wZmax + m_caps.wZmin) / 2.f) * 200.f / (m_caps.wZmax - m_caps.wZmin);
                state.axes[Joystick::R] =
                    (pos.dwRpos - (m_caps.wRmax + m_caps.wRmin) / 2.f) * 200.f / (m_caps.wRmax - m_caps.wRmin);
                state.axes[Joystick::U] =
                    (pos.dwUpos - (m_caps.wUmax + m_caps.wUmin) / 2.f) * 200.f / (m_caps.wUmax - m_caps.wUmin);
                state.axes[Joystick::V] =
                    (pos.dwVpos - (m_caps.wVmax + m_caps.wVmin) / 2.f) * 200.f / (m_caps.wVmax - m_caps.wVmin);
...
void JoystickImpl::initializeDInput()
{
        ...
        if (dinput8dll) {
                ...
                DirectInput8CreateFunc directInput8Create =
                    reinterpret_cast<DirectInput8CreateFunc>(GetProcAddress(dinput8dll, "DirectInput8Create"));

                if (directInput8Create) {
                        // Try to acquire a DirectInput 8.x interface
                        HRESULT result = directInput8Create(GetModuleHandleW(NULL), 0x0800, guids::IID_IDirectInput8W,
...
void JoystickImpl::updateConnectionsDInput()
{
        ...
        // Enumerate devices
        HRESULT result = directInput->EnumDevices(DI8DEVCLASS_GAMECTRL, &JoystickImpl::deviceEnumerationCallback, NULL,
                                                  DIEDFL_ATTACHEDONLY);
...
bool JoystickImpl::openDInput(unsigned int index)
{
        ...
        for (std::vector<JoystickRecord>::iterator i = joystickList.begin(); i != joystickList.end(); ++i) {
                if (i->index == index) {
                        ...
                        result = m_device->EnumObjects(&JoystickImpl::deviceObjectEnumerationCallback, this,
...
JoystickState JoystickImpl::updateDInput()
{
        ...
        if (m_device) {
                ...
                // If we have not acquired or have lost the device, attempt to (re-)acquire it and get the device state
                // again
                ...
                for (int i = 0; i < Joystick::ButtonCount; ++i) {
                        if (m_buttons[i] != -1) {
                                BYTE value = *reinterpret_cast<const BYTE*>(reinterpret_cast<const char*>(&joystate) +
                                                                            m_buttons[i]);
...
////////////////////////////////////////////////////////////
BOOL CALLBACK JoystickImpl::deviceObjectEnumerationCallback(const DIDEVICEOBJECTINSTANCE* deviceObjectInstance,
                                                            void* userData)
{
