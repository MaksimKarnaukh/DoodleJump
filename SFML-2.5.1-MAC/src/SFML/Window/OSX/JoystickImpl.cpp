                ...
                case kHIDPage_GenericDesktop:
                        ...
                        case kHIDUsage_GD_Hatswitch:
                                ...
                                // positions we have 8 values (0-7) plus the "null" state (8).
                                {
                                        ...
                                        if (min != 0 || max != 7) {
                                                sf::err()
                                                    << std::hex << "Joystick (vendor/product id: 0x"
                                                    << m_identification.vendorId << "/0x" << m_identification.productId
                                                    << std::dec << ") range is an unexpected one: [" << min << ", "
                                                    << max << "]" << std::endl;
                                        } else {
                                                m_hat = element;
                                        }
                        ...
                        case kHIDUsage_GD_GamePad:
                                ...
                                if (IOHIDElementGetCollectionType(element) != kIOHIDElementCollectionTypeApplication) {
                                        sf::err() << std::hex << "Gamepage (vendor/product id: 0x"
                                                  << m_identification.vendorId << "/0x" << m_identification.productId
                                                  << ") is not an CA but a 0x" << IOHIDElementGetCollectionType(element)
                                                  << std::dec << std::endl;
                                }
                                break;
...
JoystickState JoystickImpl::update()
{
        ...
        for (AxisMap::iterator it = m_axis.begin(); it != m_axis.end(); ++it) {
                ...
                float scaledValue =
                    (((physicalValue - physicalMin) * (scaledMax - scaledMin)) / (physicalMax - physicalMin)) +
                    scaledMin;
