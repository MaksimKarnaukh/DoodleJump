...
SoundStream::SoundStream()
    : m_thread(&SoundStream::streamData, this), m_threadMutex(), m_threadStartState(Stopped), m_isStreaming(false),
      m_buffers(), m_channelCount(0), m_sampleRate(0), m_format(0), m_loop(false), m_samplesProcessed(0),
      m_bufferSeeks()
{
}
...
void SoundStream::play()
{
        ...
        if (m_format == 0) {
                err() << "Failed to play audio stream: sound parameters have not been initialized (call initialize() "
                         "first)"
                      << std::endl;
                return;
        }
...
////////////////////////////////////////////////////////////
unsigned int SoundStream::getChannelCount() const { return m_channelCount; }

////////////////////////////////////////////////////////////
unsigned int SoundStream::getSampleRate() const { return m_sampleRate; }
...
////////////////////////////////////////////////////////////
void SoundStream::setLoop(bool loop) { m_loop = loop; }

////////////////////////////////////////////////////////////
bool SoundStream::getLoop() const { return m_loop; }
...
bool SoundStream::fillAndPushBuffer(unsigned int bufferNum, bool immediateLoop)
{
        ...
        for (Uint32 retryCount = 0; !onGetData(data) && (retryCount < BufferRetries); ++retryCount) {
                ...
                // Return to the beginning or loop-start of the stream source using onLoop(), and store the result in
                // the buffer seek array This marks the buffer as the "last" one (so that we know where to reset the
                // playing position)
