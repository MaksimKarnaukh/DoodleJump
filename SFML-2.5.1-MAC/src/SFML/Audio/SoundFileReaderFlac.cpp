...
namespace {
FLAC__StreamDecoderReadStatus streamRead(const FLAC__StreamDecoder*, FLAC__byte buffer[], std::size_t* bytes,
                                         void* clientData)
{
...
FLAC__StreamDecoderWriteStatus streamWrite(const FLAC__StreamDecoder*, const FLAC__Frame* frame,
                ...
                for (unsigned int j = 0; j < frame->header.channels; ++j) {
                        ...
                        if (data->buffer && data->remaining > 0) {
                                ...
                                *data->buffer++ = sample;
                                ...
                                // We are either seeking (null buffer) or have decoded all the requested samples during
                                // a normal read (0 remaining), so we put the sample in a temporary buffer until next
                                // call
...
bool SoundFileReaderFlac::open(InputStream& stream, Info& info)
{
        ...
        FLAC__stream_decoder_init_stream(m_decoder, &streamRead, &streamSeek, &streamTell, &streamLength, &streamEof,
                                         &streamWrite, &streamMetadata, &streamError, &m_clientData);
        ...
        if (left > 0) {
                if (left > maxCount) {
                        ...
                        std::copy(m_clientData.leftovers.begin(),
                                  m_clientData.leftovers.begin() + static_cast<std::size_t>(maxCount), samples);
