...
Socket::Status TcpSocket::connect(const IpAddress& remoteAddress, unsigned short remotePort, Time timeout)
{
        ...
        if (timeout <= Time::Zero) {
                ...
                if (status == Socket::NotReady) {
                        ...
                        // Wait for something to write on our socket (which means that the connection request has
                        // returned)
...
Socket::Status TcpSocket::send(const void* data, std::size_t size, std::size_t& sent)
{
        ...
        for (sent = 0; sent < size; sent += result) {
                ...
                result =
                    ::send(getHandle(), static_cast<const char*>(data) + sent, static_cast<int>(size - sent), flags);
...
Socket::Status TcpSocket::receive(Packet& packet)
{
        ...
        if (m_pendingPacket.SizeReceived < sizeof(m_pendingPacket.Size)) {
                ...
                while (m_pendingPacket.SizeReceived < sizeof(m_pendingPacket.Size)) {
                        ...
                        Status status =
                            receive(data, sizeof(m_pendingPacket.Size) - m_pendingPacket.SizeReceived, received);
        ...
        while (m_pendingPacket.Data.size() < packetSize) {
                ...
                std::size_t sizeToGet =
                    std::min(static_cast<std::size_t>(packetSize - m_pendingPacket.Data.size()), sizeof(buffer));
