...
void Socket::create(SocketHandle handle)
{
        ...
        if (m_socket == priv::SocketImpl::invalidSocket()) {
                ...
                if (m_type == Tcp) {
                        ...
                        int yes = 1;
                        if (setsockopt(m_socket, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char*>(&yes),
                                       sizeof(yes)) == -1) {
...
// On Mac OS X, disable the SIGPIPE signal on disconnection
                        ...
                        if (setsockopt(m_socket, SOL_SOCKET, SO_NOSIGPIPE, reinterpret_cast<char*>(&yes),
                                       sizeof(yes)) == -1) {
                        ...
                        int yes = 1;
                        if (setsockopt(m_socket, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<char*>(&yes),
                                       sizeof(yes)) == -1) {
