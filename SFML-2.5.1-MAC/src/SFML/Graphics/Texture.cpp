...
////////////////////////////////////////////////////////////
Texture::Texture()
    : m_size(0, 0), m_actualSize(0, 0), m_texture(0), m_isSmooth(false), m_sRgb(false), m_isRepeated(false),
      m_pixelsFlipped(false), m_fboAttachment(false), m_hasMipmap(false), m_cacheId(getUniqueId())
{
}

////////////////////////////////////////////////////////////
Texture::Texture(const Texture& copy)
    : m_size(0, 0), m_actualSize(0, 0), m_texture(0), m_isSmooth(copy.m_isSmooth), m_sRgb(copy.m_sRgb),
      m_isRepeated(copy.m_isRepeated), m_pixelsFlipped(false), m_fboAttachment(false), m_hasMipmap(false),
...
bool Texture::create(unsigned int width, unsigned int height)
{
        ...
        glCheck(glTexImage2D(GL_TEXTURE_2D, 0, (m_sRgb ? GLEXT_GL_SRGB8_ALPHA8 : GL_RGBA), m_actualSize.x,
                             m_actualSize.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));
        glCheck(
            ...
            ((area.left <= 0) && (area.top <= 0) && (area.width >= width) && (area.height >= height))) {
                ...
                if (create(rectangle.width, rectangle.height)) {
                        ...
                        for (int i = 0; i < rectangle.height; ++i) {
                                glCheck(glTexSubImage2D(GL_TEXTURE_2D, 0, 0, i, rectangle.width, 1, GL_RGBA,
...
void Texture::update(const Texture& texture, unsigned int x, unsigned int y)
{
        ...
        if (GLEXT_framebuffer_object && GLEXT_framebuffer_blit) {
                ...
                glCheck(GLEXT_glFramebufferTexture2D(GLEXT_GL_READ_FRAMEBUFFER, GLEXT_GL_COLOR_ATTACHMENT0,
                ...
                glCheck(GLEXT_glFramebufferTexture2D(GLEXT_GL_DRAW_FRAMEBUFFER, GLEXT_GL_COLOR_ATTACHMENT0,
