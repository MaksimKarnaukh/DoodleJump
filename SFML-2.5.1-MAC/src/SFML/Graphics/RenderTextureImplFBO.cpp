...
void contextDestroyCallback(void* arg)
{
        ...
        for (std::set<std::map<sf::Uint64, unsigned int>*>::iterator frameBuffersIter = frameBuffers.begin();
...
RenderTextureImplFBO::RenderTextureImplFBO()
    : m_depthStencilBuffer(0), m_colorBuffer(0), m_width(0), m_height(0), m_context(NULL), m_textureId(0),
      m_multisample(false), m_stencil(false)
{
        Lock lock(mutex);
...
RenderTextureImplFBO::~RenderTextureImplFBO()
{
        ...
        for (std::map<Uint64, unsigned int>::iterator iter = m_frameBuffers.begin(); iter != m_frameBuffers.end();
             ++iter)
...
////////////////////////////////////////////////////////////
void RenderTextureImplFBO::unbind() { glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_FRAMEBUFFER, 0)); }

////////////////////////////////////////////////////////////
bool RenderTextureImplFBO::create(unsigned int width, unsigned int height, unsigned int textureId,
...

        {
                ...
                if (!settings.antialiasingLevel) {
                        ...
                        if (settings.stencilBits) {
                                ...
                                glCheck(GLEXT_glRenderbufferStorage(GLEXT_GL_RENDERBUFFER, GLEXT_GL_DEPTH24_STENCIL8,
                                                                    width, height));
                                ...
                                glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
                                glCheck(GLEXT_glRenderbufferStorage(GLEXT_GL_RENDERBUFFER, GLEXT_GL_DEPTH_COMPONENT,
                                                                    width, height));
                        }
                } else {
                        ...
                        if (settings.stencilBits) {
                                ...
                                glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
                                glCheck(GLEXT_glRenderbufferStorageMultisample(
                                    GLEXT_GL_RENDERBUFFER, settings.antialiasingLevel, GLEXT_GL_DEPTH24_STENCIL8, width,
                                    height));
                                ...
                                glCheck(GLEXT_glBindRenderbuffer(GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
                                glCheck(GLEXT_glRenderbufferStorageMultisample(
                                    GLEXT_GL_RENDERBUFFER, settings.antialiasingLevel, GLEXT_GL_DEPTH_COMPONENT, width,
                                    height));
                        }
...
bool RenderTextureImplFBO::createFrameBuffer()
{
        ...
        if (!m_multisample && m_depthStencilBuffer) {
                glCheck(GLEXT_glFramebufferRenderbuffer(GLEXT_GL_FRAMEBUFFER, GLEXT_GL_DEPTH_ATTACHMENT,
                                                        GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));

#ifndef SFML_OPENGL_ES

                if (m_stencil) {
                        glCheck(GLEXT_glFramebufferRenderbuffer(GLEXT_GL_FRAMEBUFFER, GLEXT_GL_STENCIL_ATTACHMENT,
                                                                GLEXT_GL_RENDERBUFFER, m_depthStencilBuffer));
                }
        ...
        glCheck(GLEXT_glFramebufferTexture2D(GLEXT_GL_FRAMEBUFFER, GLEXT_GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                             m_textureId, 0));
        ...
        if (m_multisample) {
                ...
                if (!multisampleFrameBuffer) {
                        err() << "Impossible to create render texture (failed to create the multisample frame buffer "
                                 "object)"
                              << std::endl;
                        return false;
                }
                ...
                if (status != GLEXT_GL_FRAMEBUFFER_COMPLETE) {
                        ...
                        err() << "Impossible to create render texture (failed to link the render buffers to the "
...
void RenderTextureImplFBO::updateTexture(unsigned int)
{
        ...
        if (m_multisample && m_width && m_height && activate(true)) {
                ...
                if ((iter != m_frameBuffers.end()) && (multisampleIter != m_multisampleFrameBuffers.end())) {
                        ...
                        glCheck(GLEXT_glBlitFramebuffer(0, 0, m_width, m_height, 0, 0, m_width, m_height,
