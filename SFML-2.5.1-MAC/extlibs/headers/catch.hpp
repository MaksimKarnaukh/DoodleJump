...
public:
        ...
        virtual void reconstructExpression(std::string& dest) const CATCH_OVERRIDE { dest = Catch::toString(m_lhs); }
...
public:
        ...
        operator T() const
        {
                ...
                return T(); // Suppress spurious "not all control paths return a value" warning in Visual Studio - if
                            // you know how to fix this please do so
        }
...
struct Equals : StringHolder
{
        ...
        virtual std::string describe() const CATCH_OVERRIDE { return "equals string: " + Catch::toString(m_substr); }
};
...
struct Contains : StringHolder
{
        ...
        virtual std::string describe() const CATCH_OVERRIDE { return "contains string: " + Catch::toString(m_substr); }
};
...
struct StartsWith : StringHolder
{
        ...
        virtual std::string describe() const CATCH_OVERRIDE { return "starts with: " + Catch::toString(m_substr); }
};
struct EndsWith : StringHolder
{
        ...
        virtual std::string describe() const CATCH_OVERRIDE { return "ends with: " + Catch::toString(m_substr); }
};
...
struct ConfigData
{
        ...
        ConfigData()
            : listTests(false), listTags(false), listReporters(false), listTestNamesOnly(false), listExtraInfo(false),
              showSuccessfulTests(false), shouldDebugBreak(false), noThrow(false), showHelp(false),
              showInvisibles(false), filenamesAsTags(false), libIdentify(false), abortAfter(-1), rngSeed(0),
              ...
              useColour(UseColour::Auto), waitForKeypress(WaitForKeypress::Never)
        {
        }
...
class CommandLine
{
        ...
        public:
                ...
                // Bind a free function taking a single argument - the object to operate on (no placeholder string
                // required)
                template <typename C>
                ...
                // Bind a free function taking a single argument - the object to operate on (requires a placeholder
                // string)
                template <typename C, typename T>
                ...
                while (it != strEnd) {
                        ...
                        if (itEnd != strEnd) {
                                ...
                                if (!foundWrapPoint) {
                                        ...
                                        while (itEnd > it &&
                                               wrappableInsteadOfChars.find(*(itEnd - 1)) != std::string::npos)
                                                --itEnd;
                                }
                        }
...
public:
        explicit RunContext(Ptr<IConfig const> const& _config, Ptr<IStreamingReporter> const& reporter)
            : m_runInfo(_config->name()), m_context(getCurrentMutableContext()), m_activeTestCase(CATCH_NULL),
              m_config(_config), m_reporter(reporter), m_shouldReportUnexpected(true)
        {
...
private: // IResultCapture
        ...
        virtual bool lastAssertionPassed() { return m_totals.assertions.passed == (m_prevPassed + 1); }
...
public: // IRegistryHub
        ...
        virtual ITagAliasRegistry const& getTagAliasRegistry() const CATCH_OVERRIDE { return m_tagAliasRegistry; }
...
TestCaseInfo::TestCaseInfo(TestCaseInfo const& other)
    : name(other.name), className(other.className), description(other.description), tags(other.tags),
      lcaseTags(other.lcaseTags), tagsAsString(other.tagsAsString), lineInfo(other.lineInfo),
...
ResultBuilder::ResultBuilder(char const* macroName, SourceLineInfo const& lineInfo, char const* capturedExpression,
                             ResultDisposition::Flags resultDisposition, char const* secondArg)
    : m_assertionInfo(macroName, lineInfo, capturedExpression, resultDisposition, secondArg), m_shouldDebugBreak(false),
      m_shouldThrow(false), m_guardException(false), m_usedStream(false)
{
}
